...
class TimetableWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun render(context: Context, manager: AppWidgetManager, id: Int, now: LocalTime, summary: String) {
            ...
            when {
                current != null -> {
                    ...
                    views.setTextViewText(R.id.widget_subject, summary.removePrefix("${current}교시  "))
                ...
                next != null -> {
                    ...
                    views.setTextViewText(R.id.widget_subject, summary.removePrefix("다음 ${next}교시  "))
        ...
        private fun renderFallback(context: Context, manager: AppWidgetManager, id: Int) {
            try {
                val views = RemoteViews(context.packageName, R.layout.widget_layout)
                views.setTextViewText(R.id.widget_school, "군포중앙고 · 스마트 시간표")
                views.setTextViewText(R.id.widget_badge, "SMART")
                views.setTextViewText(R.id.widget_title, "시간표 준비 중")
                views.setTextViewText(R.id.widget_time, "")
                views.setTextViewText(R.id.widget_subject, "잠시 후 다시 확인해 주세요.")
                views.setTextViewText(R.id.widget_hint, "탭해서 앱 열기")
                val openIntent = Intent(context, MainActivity::class.java)
                val pending = PendingIntent.getActivity(context, id, openIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
                views.setOnClickPendingIntent(R.id.widget_root, pending)
                manager.updateAppWidget(id, views)
