...
class TimetableWidget : GlanceAppWidget() {
    ...
    private fun WidgetContent() {
        ...
        val subject = if (target == null) "오늘 수업이 모두 끝났습니다." else "시간표를 열어 과목을 확인하세요."

        Column(
            GlanceModifier.fillMaxSize().background(ColorProvider(android.graphics.Color.WHITE)),
            verticalAlignment = Alignment.Vertical.CenterVertically
        ) {
            Text("군포중앙고 · 1학년 5반", style = TextStyle(color = ColorProvider(android.graphics.Color.DKGRAY)))
            Text(title)
            Text(times, style = TextStyle(color = ColorProvider(android.graphics.Color.GRAY)))
            Text(subject, style = TextStyle(color = ColorProvider(android.graphics.Color.DKGRAY)))
        }
    }
