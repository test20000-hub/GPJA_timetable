...
### v2.0.9 — Android 업데이트 서명 고정 및 위젯 추가 안정화
- Android APK를 GitHub Actions 캐시 기반 debug 서명에서 고정 release keystore 기반 서명으로 전환했습니다.
- 같은 release keystore로 빌드된 APK끼리는 기존 설치본을 삭제하지 않고 정상적으로 업데이트할 수 있도록 CI를 변경했습니다.
- APK 빌드는 `assembleRelease`를 사용하며 서명용 GitHub Secrets가 없으면 호환되지 않는 APK를 만들지 않고 명확하게 실패하도록 변경했습니다.
- 홈 화면 위젯 추가 시 `onUpdate()`에서 네트워크를 동기적으로 기다리지 않도록 변경해 런처의 추가 작업이 멈추지 않게 했습니다.
- 기존 캐시 시간표를 먼저 표시하고 네트워크 시간표 갱신은 백그라운드에서 수행하도록 변경했습니다.
- Android 앱 `versionCode`를 `29`, `versionName`을 `2.0.9`로 갱신했습니다.

### v2.0.8 — Android 위젯 추가 실패 방어
- 홈 화면에 위젯을 추가할 때 시간표 네트워크 조회가 실패해도 위젯 프로바이더가 종료되지 않도록 예외 처리를 추가했습니다.
- 시간표 서버가 일시적으로 응답하지 않아도 위젯이 검은 화면/추가 실패 상태가 되지 않고 안전한 안내 문구를 표시하도록 수정했습니다.
- Android 앱 `versionCode`를 `28`, `versionName`을 `2.0.8`로 갱신했습니다.
- 사이트 버전은 `v2.0.7`을 유지합니다.
