...
### v2.0.8 — 2.0.9 롤백 및 위젯 추가 문제 재수정
- Android 앱 버전을 `versionCode 28`, `versionName 2.0.8`로 롤백했습니다.
- v2.0.9의 고정 release keystore 의존성을 제거하고 기존 v2.0.8 debug 빌드 방식으로 복구했습니다.
- 위젯 설치 시 설정 Activity를 거치지 않고 즉시 위젯을 생성하도록 변경했습니다.
- 위젯 최초 렌더링에서는 네트워크 요청을 전혀 수행하지 않고 즉시 안전한 UI를 표시하도록 유지했습니다.
- 시간표 데이터 갱신은 최초 위젯 생성 이후 백그라운드에서 수행하도록 수정했습니다.
- 위젯 `initialLayout`을 실제 위젯 레이아웃으로 변경해 런처에서 검은/빈 초기 화면이 표시되는 문제를 줄였습니다.
