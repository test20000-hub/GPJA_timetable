...
- 급식 API 요청에 10초 타임아웃과 요청 순번 검사를 추가하여 오래 걸린 이전 요청이 최신 날짜 화면을 덮어쓰지 않도록 수정
- 급식 페이지의 주말 자동 이동을 제거하여 사용자가 선택한 날짜를 그대로 조회하도록 수정
- 재시도 버튼을 인라인 `onclick`에서 이벤트 리스너 방식으로 변경하여 함수 스코프 문제 제거
- 급식 페이지 `meal.js` 캐시 버스터를 v9로 갱신하고 서비스 워커 캐시를 v15로 갱신
- 사이트 및 급식 페이지 푸터/메타 버전을 v2.0.4로 동기화
