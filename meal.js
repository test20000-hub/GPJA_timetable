...
function moveDay(direction){const next=new Date(current);do{next.setDate(next.getDate()+direction)}while(isWeekend(next));current=next;load()}
const weekCard=document.querySelector('.week-card');
let touchStartX=0,touchStartY=0,touchActive=false;
weekCard?.addEventListener('touchstart',e=>{if(e.target.closest('button'))return;const t=e.changedTouches[0];touchStartX=t.clientX;touchStartY=t.clientY;touchActive=true},{passive:true});
weekCard?.addEventListener('touchend',e=>{if(!touchActive)return;touchActive=false;const t=e.changedTouches[0];const dx=t.clientX-touchStartX,dy=t.clientY-touchStartY;if(Math.abs(dx)<55||Math.abs(dx)<=Math.abs(dy))return;moveDay(dx<0?1:-1)},{passive:true});
