...
PARAMS={'KEY':KEY,'Type':'json','ATPT_OFCDC_SC_CODE':'J10','SD_SCHUL_CODE':'7530174','AY':str(datetime.now().year),'pSize':'1000'}

def fetch(page):
    q=PARAMS|{'pIndex':str(page)}
...
def main():
    ...
    for page in range(1,20):
        batch=rows_from(fetch(page)); rows.extend(batch)
